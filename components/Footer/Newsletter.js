  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
      ...
      <div className="flex flex-col justify-center w-full max-w-2xl min-h-[44px]">
        {status === "success" ? (
          <div className="text-green-400/90 text-[14px] font-medium animate-fade-in flex items-center gap-2 bg-white/[0.05] border border-white/10 rounded-lg px-6 py-2 w-full h-[40px] sm:h-auto">
          ...
          <div className="relative w-full">
            ...
            >
              <input
                ...
                className="flex-[3] bg-transparent px-6 py-2 text-white placeholder:text-gray-500 focus:outline-none text-[14px]"
                required
              />
              <button
                type="submit"
                disabled={status === "loading"}
                className="flex-1 bg-[#975554] hover:bg-white hover:text-[#975554] text-white px-4 py-2 flex items-center justify-center gap-2 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed whitespace-nowrap text-[14px] font-bold"
              >
                {status === "loading" ? (
                  "..."
                ) : (
