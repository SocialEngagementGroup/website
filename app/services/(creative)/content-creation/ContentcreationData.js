export const contentCreationData = {
  hero: {
    title: "Content Creation",
    description:
      "Create content that converts. From videos and graphics to influencer and AI-generated assets—build engagement, trust, and growth for your brand.",
    subtitle: "Creative",
    ctaButton: "Connect With Us",
        ctaLink: "#contact",
   img: "/assets/services/creative/branding/heroimg.png",
   bg: "/assets/background/service-bg.png",
  },
  intro: {
    heading: "Content That Captures Attention",
    description:
      "Exceptional content drives clicks, shares, and conversions. We build a tailored content strategy rooted in deep audience insight then create the assets that make it work: scroll-stopping design, high-impact video, authentic influencer collaborations, and fast, scalable AI content. Every piece is engineered for the platform and purpose, from viral short-form and thought-leadership series to campaign-ready visuals. We don’t just produce content; we craft brand experiences that spark conversation, build trust, and deliver measurable ROI.",
  },
  cta: {
   bg: "/assets/services/cta/Digital-google-business-profile.png",
    title: "<strong>Grow</strong> your business <br/> with <strong>SEG</strong> ",
    button: "Book a call",
    link: "https://calendly.com/itseg/segmeet",
  },
  Costof: {
    heading1: "Why Businesses Need Professional Content Creation",
    heading2: "The Cost of Weak Content",
    description:
      "High-quality visual content and engaging brand storytelling are essential for cutting through the digital noise. They are proven to boost your online reach, build brand trust, and increase conversion rates. Without a professional content strategy, your marketing messages risk being ignored, and your investment goes to waste.",
    items: [
      {
       img: "/assets/services/creative/creative-content/audience-engagement.png",
        text: "Your posts and ads get lost in crowded social media feeds, leading to low <strong>audience engagement.</strong>",
      },
      {
       img: "/assets/services/creative/creative-content/memorable-impact.png",
        text: "Your campaigns feel <strong>generic</strong> and fail to make a memorable impact.",
      },
      {
       img: "/assets/services/creative/creative-content/you-struggle-to-build-a-loyal-community.png",
        text: "Your <strong>brand message</strong> gets diluted, and you struggle to build a loyal community.",
      },
      {
       img: "/assets/services/creative/creative-content/win-the-online-conversation-and-market-share.png",
        text: "<strong>Competitors</strong> with stronger content easily win the online conversation and market share.",
      },
    ],
  },
  ProfessionalContent: {
    heading: "What Happens With Strategic Content",
    items: [
      {
        text: "Your posts become <strong>engaging content</strong> that sparks instant comments, likes, and shares.",
      },
      {
        text: "Strategic <strong>video marketing</strong> boosts your brand's visibility, reach, and conversions.",
      },
      {
        text: "Authentic <strong>influencer campaigns</strong> expand your reach and build powerful social trust.",
      },
      {
        text: "Innovative <strong>AI-powered content</strong> keeps your brand at the forefront of industry trends.",
      },
    ],
    video: "/assets/videos/services/creative/Content-Creation.mp4",
  },
  ContentStrategy: {
    heading: "What You Get",
    items: [
      {
        text: "Custom <strong>branded graphics</strong> for social media, ads, blogs, and marketing campaigns.",
      },
      {
        text: "Professional <strong>short-form</strong> and <strong>long-form video production</strong> and editing.",
      },
      {
        text: "Strategic <strong>Influencer collaboration </strong> management, from outreach to campaign execution.",
      },
      {
        text: "Scalable <strong>AI-powered content</strong> tailored to your specific niche and brand voice.",
      },
    ],
   img: "/assets/services/creative/creative-content/creative-content-creation.png",
  },
  FAQ: {
    heading: "FAQs",
    items: [
      {
        question: "1. What types of content do you create?",
        answer:
          "We produce branded graphics, social content, videos, influencer campaigns, and AI-generated assets tailored to your industry.",
      },
      {
        question: "2. Why should I invest in professional content creation?",
        answer:
          "High-quality content captures attention, boosts engagement, and converts viewers into customers.",
      },
      {
        question: "3. Can you create both short-form and long-form video?",
        answer:
          "Yes, we handle everything from TikTok/Reels to corporate videos and brand films.",
      },
      {
        question: "4. Do you offer AI-generated content?",
        answer:
          "Yes, we use AI tools to generate scalable, trend-driven content that saves time while staying on brand.",
      },
      {
        question: "5. Can you work with influencers for my brand?",
        answer:
          "Absolutely. We connect you with the right influencers to expand reach and build trust.",
      },
      {
        question: "6. How often should I create new content?",
        answer:
          "Consistency is key. Most brands see results with a steady stream of content (weekly or monthly) aligned to their campaigns.",
      },
    ],
  },
};
